"""
this file generate the analysis of the nuclear norm analysis
"""
...
# Define a nuclear norm loss function
def nuclear_norm_loss(y_true, y_pred):
    # Compute the squared differences between generated and input images
    diff = tf.square(y_pred - y_true)
    # Compute the SVD of the squared differences and sum up the singular values
    s, _, _ = tf.linalg.svd(diff)
    nuclear_norm = tf.reduce_sum(s)
    return nuclear_norm

# Define the projection function with the custom loss function
def argminZ(image):
    mainList = []
    ZList = []
    for i in range(R):
        myloss = nuclear_norm_loss  # Use the custom loss function
...
def ld_mnist(batch=150):
    ...
    mnist_test = mnist_test.map(convert_types).shuffle(1000)
    mnist_test = mnist_test.take(2000)
